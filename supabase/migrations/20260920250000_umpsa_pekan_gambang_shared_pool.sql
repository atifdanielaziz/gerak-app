-- UMPSA's two campuses (Pekan and Gambang) are close enough that riders are
-- expected to cover both — but every customer-facing rider-matching query
-- filtered by the EXACT campus string, so a rider assigned only to Pekan
-- never showed up for a Gambang customer and vice versa, even though
-- get_jubah_riders_directory_v2's own preview (fed the whole university's
-- campus list already) made it look like campus didn't matter.
--
-- jubah_campus_match_group() is intentionally narrow: it only widens
-- Pekan/Gambang into each other. Every other university's campuses are
-- often geographically far apart (e.g. UiTM's Machang vs Shah Alam/Puncak
-- Alam, UMS's Labuan island vs Kota Kinabalu) where a rider covering one
-- genuinely cannot cover the other — so this does NOT generalize to "match
-- by university" everywhere, only to this specific, deliberately-chosen
-- pair.
create or replace function public.jubah_campus_match_group(p_campus text)
returns text[]
language sql
immutable
as $$
  select case
    when p_campus in ('Pekan', 'Gambang') then array['Pekan', 'Gambang']
    else array[p_campus]
  end;
$$;
