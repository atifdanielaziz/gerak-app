-- Temporary, read-only — checks whether any EXISTING ride_orders row would
-- fail the new validate_ride_order_fare trigger, before it goes live on
-- real traffic. Dropped immediately after being queried once.
...
$$;
revoke all on function public.diag_check_existing_fares() from public, anon, authenticated;
