-- ============================================================
-- create_jubah_booking: stop returning raw Postgres error text to guests
-- Run in: Supabase Dashboard > SQL Editor > New query
-- ============================================================
--
-- The catch-all exception handler returned sqlerrm verbatim to the caller
-- — this is an anon-callable, unauthenticated RPC, so any unexpected
-- constraint/type failure leaked internal column/table/constraint names
-- straight to a guest. The one legitimate use of the raw text (Jubah.tsx
-- pattern-matching "duplicate key|unique constraint" to silently retry with
-- a fresh reference) is replaced with a stable 'code' field instead of a
-- raw-text regex, so that behavior keeps working without needing the
-- underlying Postgres message. The real error is still logged server-side
-- (visible in Supabase's function/database logs) via RAISE WARNING.
