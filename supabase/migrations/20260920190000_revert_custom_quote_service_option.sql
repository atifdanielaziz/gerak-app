-- Reverts the DB side of 20260920180000 (Service Option on Custom Quote)
-- to match the frontend revert of that same commit. Restores
-- create_jubah_custom_quote/resolve_jubah_custom_quote/
-- resolve_jubah_custom_quote_by_ic to their state as of 20260920170000
-- (phone + auto-campus/rider, no service option).

drop function if exists public.create_jubah_custom_quote(text, numeric, text, text, text);
