-- Let an admin/superadmin double as a real, bookable Jubah rider on the
-- SAME account — the same convenience Gerak Car already has for drivers
-- (find_driver_search.sql: "role = 'driver' or (role in ('admin',
-- 'superadmin') and can_drive)"). Before this, the Staff dropdown's
-- Robe/Daily toggle and every rider-eligibility check in the Jubah pipeline
-- required role = 'rider' with no such exception, so toggling can_robe on
-- an admin did nothing — RiderHome.tsx already special-cased admin/
-- superadmin favorably (exempt from the docs-approval and active-status
-- gates), but the backend never actually let them become assignable.
--
-- set_rider_capabilities (which the Staff dropdown's Robe/Daily toggle
-- calls) already had no role restriction at all — only these three needed
-- widening.

-- 1) set_rider_jubah_assignment — admin assigns a campus/method/drop-point
-- to a rider; the target lookup required role = 'rider'.
...
begin
  ...
  where id = p_user_id and (role = 'rider' or role in ('admin', 'superadmin'));
...
-- 2) get_active_jubah_riders — powers the customer "Select Rider" dropdown
-- and the default/custom booking rider-eligibility checks.
...
as $$
  ...
  where ja.campus    = p_campus
    ...
    and (p.role = 'rider' or p.role in ('admin', 'superadmin'))
    and p.can_robe   = true
    and p.status     = 'active'
  order by p.name;
$$;
...
-- 3) create_custom_jubah_booking — re-validates the chosen rider server-side
-- before inserting the booking (see 20260920090000_simplify_jubah_custom_quotes.sql).
...
begin
  ...
  if v_rider is null or not exists (
    select 1 from public.profiles p join public.jubah_rider_assignments a on a.rider_id = p.id
     where p.id = v_rider and (p.role = 'rider' or p.role in ('admin', 'superadmin'))
