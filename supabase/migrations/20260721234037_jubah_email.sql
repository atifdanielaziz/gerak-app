-- ============================================================
-- Migration: Collect customer email, use it as ToyyibPay's billEmail
-- Jubah booking never collected an email, so toyyibpay-create-bill has
-- been sending ToyyibPay a placeholder (jubah@atepgerak.app) as billEmail
-- — meaning any receipt/notification ToyyibPay itself might send based on
-- that field goes to our placeholder inbox, never reaching the customer.
-- This adds a real email column and threads it through so the real
-- customer email reaches ToyyibPay's bill instead.
--
-- Also restores an explicit status = 'ordered' in the INSERT.
-- migration_jubah_pricing_per_university.sql's create_jubah_booking
-- dropped it — the table's own DEFAULT should still cover this
-- independently, but since this function is being touched anyway, this
-- closes the gap with certainty rather than relying on that holding.
