...
export const VerifyDocsTab = forwardRef<VerifyDocsTabHandle, VerifyDocsTabProps>(function VerifyDocsTab(
  ...
  return <div className="flex flex-col gap-4">
    <div className="bg-white border border-slate-100 rounded-2xl p-3.5 flex flex-col gap-2">
      ...
      <AdminSearchInput value={search} onChange={setSearch} placeholder="Name or Gerak ID" />
    ...
    <section className="bg-white border border-slate-100 rounded-3xl p-5">
      ...
      : <table className="min-w-[760px] w-full border-collapse text-left text-xs"><thead className="sticky top-0 z-10 bg-white"><tr className="text-slate-400">{['Name','Role','Gerak ID','Campus','Email','Status',''].map(h => <th key={h} className="py-2.5 pr-4 font-semibold border-b border-slate-100 whitespace-nowrap">{h}</th>)}</tr></thead><tbody>{filtered.map(d => <tr key={d.id} onClick={() => onViewProfile(profileUser(d))} className="border-b border-slate-100 last:border-b-0 active:bg-slate-50 cursor-pointer">
        ...
        <button type="button" data-axis-lock-ignore onClick={e => { e.preventDefault(); e.stopPropagation(); const rect = e.currentTarget.getBoundingClientRect(); setMenuPosition({ top: Math.max(8, Math.min(rect.bottom + 4, window.innerHeight - 224)), left: Math.max(8, Math.min(rect.right - 190, window.innerWidth - 198)) }); setOpenMenuId(v => v === d.id ? null : d.id); }} className="w-8 h-8 flex items-center justify-center rounded-lg text-slate-400 hover:bg-slate-100 active:scale-90 transition-transform transform-gpu" aria-label={`Actions for ${d.name}`} aria-expanded={openMenuId === d.id}><MoreVertical className="w-4 h-4" /></button>
        {openMenuId === d.id && createPortal(<><div className="fixed inset-0 z-[9998]" onPointerDown={e => { e.preventDefault(); e.stopPropagation(); setOpenMenuId(null); }} /><div data-axis-lock-ignore onClick={e => e.stopPropagation()} className="fixed z-[9999] min-w-[190px] max-h-[15rem] overflow-y-auto overscroll-contain touch-pan-y bg-white border border-slate-100 rounded-2xl shadow-xl pointer-events-auto [-webkit-overflow-scrolling:touch]" style={{ top: menuPosition.top, left: menuPosition.left }}>
          {d.license_url ? <a href={d.license_url} target="_blank" rel="noopener noreferrer" onClick={() => setOpenMenuId(null)} className="flex items-center gap-3 px-4 py-3 text-xs font-semibold text-slate-600 active:bg-slate-50"><ExternalLink className="w-4 h-4 text-slate-400" /> View License</a> : <div className="flex items-center gap-3 px-4 py-3 text-xs font-semibold text-slate-300" aria-disabled="true"><ExternalLink className="w-4 h-4" /> License Not Uploaded</div>}
          {d.docs_status !== 'approved' && <button type="button" disabled={!d.license_url} onClick={() => { setOpenMenuId(null); void approve(d.id); }} className="w-full flex items-center gap-3 px-4 py-3 text-xs font-semibold text-emerald-600 active:bg-slate-50 disabled:text-slate-300 disabled:cursor-not-allowed"><ShieldCheck className="w-4 h-4" /> Approve</button>}
