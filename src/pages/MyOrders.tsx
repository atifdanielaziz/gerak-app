...
import {
  ...
  User, Hash, ShieldCheck, XCircle, RotateCcw, FileDown,
} from 'lucide-react';
...
// ── PDF receipt helper ────────────────────────────────────────────────────────
const generatePdf = (o: RideOrder) => {
  const totalFare = o.fare === 'TBC' ? 'TBC' : `RM${(Number(o.fare) + (o.night_charge ?? 0)).toFixed(2)}`;
  const html = `<!DOCTYPE html><html><head><meta charset="utf-8"/>
<title>Gerak Car Receipt - ${o.id.slice(0,8).toUpperCase()}</title>
<style>
body{font-family:monospace;font-size:13px;color:#1e293b;max-width:400px;margin:40px auto;padding:0 20px}
h1{font-size:20px;font-weight:300;margin:0 0 4px}h1 span{color:#ef4444}
.sub{font-size:11px;color:#94a3b8;margin-bottom:24px}
.row{display:flex;justify-content:space-between;margin-bottom:6px}
.lbl{color:#94a3b8}.val{font-weight:700;text-align:right}
hr{border:none;border-top:1px dashed #cbd5e1;margin:12px 0}
.total{font-size:16px}.ref{font-size:10px;color:#94a3b8;text-align:center;margin-top:24px}
</style></head><body>
<h1>ger<span>a</span>k</h1>
<div class="sub">Gerak Car — Trip Receipt</div>
<div class="row"><span class="lbl">Booking Ref</span><span class="val">#${o.id.slice(0,8).toUpperCase()}</span></div>
<div class="row"><span class="lbl">Status</span><span class="val">${o.status.replace('_',' ').toUpperCase()}</span></div>
<hr/>
<div class="row"><span class="lbl">Date</span><span class="val">${o.date}</span></div>
<div class="row"><span class="lbl">Time</span><span class="val">${o.time}${o.night_charge > 0 ? ' (Night +RM5)' : ''}</span></div>
<div class="row"><span class="lbl">Campus</span><span class="val">UMPSA ${o.campus}</span></div>
<div class="row"><span class="lbl">Pick-up</span><span class="val">${o.pickup}</span></div>
<div class="row"><span class="lbl">Destination</span><span class="val">${o.destination}</span></div>
<div class="row"><span class="lbl">Passengers</span><span class="val">${o.passengers} pax</span></div>
<div class="row"><span class="lbl">Contact</span><span class="val">${o.contact}</span></div>
${o.notes ? `<div class="row"><span class="lbl">Remark</span><span class="val">${o.notes}</span></div>` : ''}
<hr/>
<div class="row total"><span class="lbl">Est. Fare</span><span class="val">${totalFare}</span></div>
${o.driver_name ? `<hr/><div class="row"><span class="lbl">Driver</span><span class="val">${o.driver_gerak_id ?? o.driver_name}</span></div>` : ''}
<div class="ref">Generated by Gerak · ${new Date().toLocaleDateString('en-MY',{day:'2-digit',month:'short',year:'numeric'})}</div>
</body></html>`;
