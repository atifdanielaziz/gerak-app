    ...
    <div className="flex-grow bg-white overflow-y-auto no-scrollbar pb-4 px-5 flex flex-col gap-5 animate-fade-in">
      ...
      <div className="bg-gradient-to-br from-slate-900 via-blue-950 to-blue-900 px-5 pt-6 pb-4 relative overflow-hidden">
        ...
        <div className="relative z-10 flex flex-col gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-xl font-bold text-white m-0 leading-tight">Gerak Transporter</h2>
              <p className="text-xs text-blue-300/80 font-semibold mt-0.5">
                Penghantaran Motosikal & Pindah Barang
              </p>
            </div>
          </div>

          <p className="text-xs text-white/80 font-normal leading-relaxed max-w-xs">
          ...
          <div className="flex bg-black/30 border border-white/10 rounded-2xl p-1 gap-1 mt-1">
            {(['Penghantaran Motosikal', 'Pindah Barang'] as const).map(s => (
              <button key={s} onClick={() => setServiceType(s)}
                className={`flex-1 py-2 rounded-xl text-xs font-semibold transition active:scale-95 flex items-center justify-center gap-1.5 ${
                  serviceType === s ? 'bg-white text-slate-900' : 'text-blue-300/70'
                }`}>
      ...
      <div className="px-4 flex flex-col gap-4">
        ...
        <div className="bg-white border border-slate-100 rounded-3xl overflow-hidden">
          ...
          >
            <span className="text-sm font-bold text-slate-700">Perkhidmatan Kami</span>
            {showFeatures
        ...
        <div className="bg-white border border-slate-100 rounded-3xl p-5 flex flex-col gap-4">
          <h3 className="text-sm font-bold text-slate-700">Penyedia Perkhidmatan</h3>
          ...
          <div className="border border-slate-100 rounded-2xl overflow-hidden">
            <div className="bg-gradient-to-r from-slate-900 to-blue-900 px-4 py-3 flex items-center gap-3">
              ...
              <div className="flex-1 min-w-0">
                ...
                <p className="text-xs text-blue-300/80 font-semibold mt-0.5">{PROVIDER.tagline}</p>
              </div>
              <span className="shrink-0 bg-emerald-400/20 border border-emerald-400/30 text-emerald-300 text-[8px] font-semibold px-2 py-0.5 rounded-full uppercase tracking-wider">
                Aktif
              </span>
            </div>
            ...
            <div className="p-4 flex flex-col gap-4">
              <div className="grid grid-cols-2 gap-2">
                {[
                  ...
                  <div key={label} className="flex flex-col gap-0.5">
                    ...
                    <span className="text-xs font-normal text-slate-700">{value}</span>
                  </div>
                ))}
              </div>
              ...
              <div className="flex flex-wrap gap-1.5 pt-1 border-t border-slate-50">
                {['Selamat Dijamin', 'Pantas & Tepat Masa', 'Servis Mesra', 'Ikatan Selamat'].map(g => (
                  <span key={g} className="text-[8px] font-semibold bg-slate-100 text-slate-500 px-2 py-0.5 rounded-full">
                    ✓ {g}
                  </span>
                ))}
              </div>
              ...
              <div className="flex gap-2 pt-1 items-center">
                ...
                <a
                  ...
                  className="w-12 h-12 flex items-center justify-center bg-slate-100 border border-slate-100 text-slate-700 rounded-2xl active:scale-95 transition shrink-0"
                >
        ...
        {/* ── HOW IT WORKS ── */}
        <div className="bg-white border border-slate-100 rounded-3xl p-5 flex flex-col gap-4">
          <h3 className="text-sm font-bold text-slate-700">Cara Tempahan</h3>

          <div className="flex flex-col gap-4">
            {STEPS.map(({ n, label, desc }, i) => (
              <div key={n} className="flex gap-3">
                <div className="flex flex-col items-center shrink-0">
                  <div className="w-8 h-8 rounded-full bg-slate-900 text-blue-300 text-xs font-semibold flex items-center justify-center">
                    {n}
                  </div>
        ...
        <div className="bg-slate-900 border border-slate-800 rounded-3xl p-5 flex items-start gap-3">
          ...
          <div>
            <p className="text-xs font-bold text-white">Harga Berdasarkan Jarak</p>
        ...
        >
          <div className="flex items-center gap-3">
            ...
            <div className="text-left">
              ...
              <p className="text-xs text-blue-300/80 font-normal mt-0.5">{serviceType}</p>
            </div>
          </div>
